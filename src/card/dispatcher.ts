 ...
 * a callback that should be forwarded back to the agent (KSGC) instead
 ...
 */
const AGENT_CALLBACK_MARKER = '__ksgc_cb';
...
export async function handleCardAction(deps: CardDispatchDeps): Promise<void> {
  ...
  // Agent-driven callback: the button was rendered by ksgc itself via
  // lark-cli, with `__ksgc_cb` set on the value. Forward the click back
  // into the scope's pending queue so ksgc resumes its session and sees
...
function forwardToAgent(
  ...
  // Strip the marker so ksgc only sees the meaningful fields it set.
